...

{
  ...
  home-manager.users.annie =
    ...
    let
      ...
      # Helper function to generate an activation script for a non-symlinked shortcut
      mkActivationShortcut = targetPath: entryOpts: ''
            DESKTOP_FILE="$HOME/${targetPath}"
            $DRY_RUN_CMD rm -f "$DESKTOP_FILE"
            $DRY_RUN_CMD cat << 'EOF' > "$DESKTOP_FILE"
        ${mkDesktopEntry entryOpts}EOF
    ...
    in
    {
      ...
      home.activation.createDesktopShortcuts = config.lib.dag.entryAfter [ "writeBoundary" ] ''
        ${mkActivationShortcut "Desktop/IN.desktop" {
          name = "IN";
          comment = "Open IN.odt in LibreOffice";
          exec = ''xdg-open "/home/annie/Dropbox/Documents/£/IN.odt"'';
          icon = "${./open_door.png}"; # Or use standard icon string like "libreoffice-writer"
          categories = "Office;";
        }}

        ${mkActivationShortcut "Desktop/Recipes.desktop" {
          name = "Recipes";
